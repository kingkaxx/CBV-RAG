...
def _popqa(split: str, cache_dir: str, limit: Optional[int]) -> List[QAExample]:
    ...
    ds = load_dataset("akariasai/PopQA", split=actual_split, cache_dir=cache_dir)
    rows = ds.select(range(min(limit, len(ds)))) if limit else ds
    out = []
    for i, ex in enumerate(rows):
...
def build_kb_rows(examples: Iterable[QAExample], chunk_words: int, overlap_words: int) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    for ex in examples:
        qa_rows.append(
            {
                "qid": normalize_ws(ex.qid),
                "dataset": normalize_ws(ex.dataset),
                "split": normalize_ws(ex.split),
                "question": normalize_ws(ex.question),
                "answers": [normalize_ws(a) for a in ex.answers],
                "support_titles": [normalize_ws(t) for t in ex.support_titles],
                "support_sentences": [normalize_ws(s) for s in ex.support_sentences],
            }
        )
