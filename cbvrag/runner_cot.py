...
from cbvrag.prompts_cot import answer_prompt, counterfactual_prompt, verify_prompt
...
def default_budgets() -> Dict[str, int]:
    return {
        ...
        "final_answer_max_new_tokens": 128,  # CoT
    }
...
def execute_action(state: EpisodeState, action: Action, controller: Any, tools: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if action in (Action.RETRIEVE_MORE_SMALL, Action.RETRIEVE_MORE_LARGE):
        ...
        answer, usage = llm.generate(
            prompt,
            max_new_tokens=int(state.budgets.get("final_answer_max_new_tokens", 128)),
            temperature=0.0,
            name="answer",
        )
        state.metrics["llm_calls"] += 1
